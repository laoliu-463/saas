...
| 商品库快速寄样把快照主键误按商品主键校验 | fixed-local | `harness/reports/current/latest-talent-address-product-sample-diagnosis.md`、`harness/reports/current/latest-quick-sample-snapshot-address-fix.md` | DDD 前置检查已改为按 `product_snapshot.id` 查询快照；“快照存在、商品主表无同主键”回归测试、本地构建、real-pre 后端重启与健康检查均 PASS。远端未部署，且未发起会调用真实第三方的寄样请求。 |
