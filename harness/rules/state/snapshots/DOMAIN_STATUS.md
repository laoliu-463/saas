...
- DDD 优化下一步：#35 用户分配、渠道、组织归属 Application 收口；同时继续推进 `DataScopeResolver` / `PermissionChecker` 消费统一。U-14 在指定测试账号或授权窗口内补 authenticated real-pre 改密 API/E2E。
- 标记：P0。
...
- DDD 优化下一步：#35 用户分配、渠道、组织归属 Application 收口；所有未来任务必须按 git-change-control.md 执行 12 条 Git 强约束。
- 标记：P0。
