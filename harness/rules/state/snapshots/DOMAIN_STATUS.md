...
## 用户域
- 最新小切片：DDD-USER-MASTER-DATA-PERMISSION-POLICY 已将 `UserMasterDataService` 的主数据角色编码归一、下拉角色过滤和跨部门管理员判断委托给用户域 `CurrentUserPermissionPolicy.normalizeRoleCodes` / `hasAnyRole`。本轮不改 `channels` / `recruiters` / `group-members` 查询 Mapper、active 用户过滤、关键词、limit、排序或真实数据。结论报告：`harness/reports/evidence-20260622-120401.md`；中间报告 `harness/reports/evidence-20260622-105205.md` 仅记录 `agent-do` 在 build/restart/health 后业务验证前超时的 FAIL 事实。风险变化：`UserMasterDataServiceTest` 覆盖 `" ADMIN "` 跨部门访问路径，`DddUserMasterDataPermissionPolicyBoundaryTest` 防止 `roleCodes.stream()` / `roleCodes.contains(RoleCodes.ADMIN)` 回流。
- 上一小切片：DDD-TALENT-RELEASE-PERMISSION-POLICY 已将 `TalentService.release` 中管理员释放判断委托给用户域 `CurrentUserPermissionPolicy.hasAnyRole`，达人服务内本地 `hasRole` / `roleCodes.stream()` 角色 matcher 已清零。本轮不改达人认领记录选择、释放状态流转、owner 快照、操作日志、达人池、归属覆盖或真实数据。报告：`harness/reports/evidence-20260621-222220.md`。
