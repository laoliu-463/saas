...
- 风险变化：`PerformanceMetricsQueryService` 已消费用户域数据范围 policy；`DashboardService`、`DataApplicationService`、`TalentQueryService.detail` 与 `TalentService.page` 已完成子路径收口；`TalentService.evaluateExclusive` / `assertCanOperateBlacklist` 等其它手写数据范围分支仍需继续分类收口。
...
- 风险变化：订单归因查询侧已消费用户域数据范围 policy；`DashboardService`、`DataApplicationService`、`PerformanceMetricsQueryService`、`TalentQueryService.detail` 与 `TalentService.page` 已在后续切片收口；`TalentService.evaluateExclusive` / `assertCanOperateBlacklist` 等其它手写数据范围分支需继续分类收口。
...
## 达人域
- 最新边界变化：`TalentService.page` 达人列表数据范围新增灰度开启的用户域 `DataScopePolicy` 路径，默认关闭仍走 Legacy `talent_claim` user/dept 取 ID 集合；本轮未改达人认领规则、达人池、保护期、归属覆盖、标签、地址、黑名单、独家评估、第三方接口或真实数据。报告：`harness/reports/evidence-20260622-144728.md`。
- 近期边界变化：`TalentQueryService.detail` 达人详情访问数据范围已新增灰度开启的用户域 `DataScopePolicy` 路径；`TalentService.release` 管理员角色编码匹配、`TalentQueryService.assertCanOperate` 操作访问角色匹配已委托用户域 policy；达人归属覆盖已通过用户域 `loadUserOwnershipReferencesByIds` 校验目标负责人存在，不再读取完整用户 DTO，既有达人认领记录 `deptId` 写入行为不变。
