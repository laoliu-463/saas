...
## 用户域
- 最新小切片：DDD-USER-ORDER-SERVICE-DATASCOPE-APPLYTO 已将 `OrderService` 灰度开启后的订单列表 / 统计数据范围过滤路径委托给用户域 `DataScopePolicy.applyTo`，移除新路径本地 `DataScopePolicy.Decision` switch；`ddd.refactor.data-scope-policy.enabled=false` 默认关闭，Legacy switch 兜底保留。本轮不改订单事实、归因、金额、接口参数、默认开关或真实数据。报告：`harness/reports/evidence-20260622-140151.md`。风险变化：`OrderServiceTest` 覆盖默认关闭旧路径、灰度开启 `LambdaQueryWrapper` / `QueryWrapper` 路径和源码边界，`DataScopePolicyParityTest` 继续覆盖 policy 与旧过滤行为等价。
