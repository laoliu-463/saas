...
## 用户域
- 最新小切片：DDD100 #35 已将用户业务组成员变更从 `SysUserMapper` / `SysUser` 直接依赖下沉到 `UserGroupMembershipStore` 端口和 `SysUserGroupMembershipStoreAdapter`；`SysUserMapper.updateDeptById` 显式覆盖 `dept_id` 写入与清空，兼容服务 `assignUsersToGroup/removeUsersFromGroup` 继续委托用户域 Application。本轮不改 Controller API、DB schema、默认 real-pre 配置或真实用户数据。报告：`harness/reports/2026-06-21/ddd-user-assign-035/evidence-20260627-134900-user-assign-org-membership.md`。风险变化：组织成员变更应用层 Mapper/Entity 依赖已清理；Auth/Role/Menu 应用层遗留持久化依赖留给 #36/#37。
- 最新小切片：DDD100 #34 已补 SysUser CRUD true-route 委托证据，`auth.service.SysUserService` 的 getById/create/update/delete/resetPassword 由 `SysUserCRUDApplicationA/B` 承接，并修正 A/B 过期类注释；本轮不改 Controller API、DB schema、默认 real-pre 配置或真实用户数据。报告：`harness/reports/2026-06-21/ddd-user-crud-034/evidence-20260627-123000-sysuser-crud-application.md`。风险变化：已证明兼容服务 CRUD 路径进入用户域 Application；authenticated real-pre 用户 CRUD 写入 E2E 仍需专用账号窗口。
...
- DDD 优化下一步：#36 角色菜单与 PermissionPolicy 收口；同时继续推进 `DataScopeResolver` / `PermissionChecker` 消费统一。U-14 在指定测试账号或授权窗口内补 authenticated real-pre 改密 API/E2E。
- 标记：P0。
...
- DDD 优化下一步：#36 角色菜单与 PermissionPolicy 收口；所有未来任务必须按 git-change-control.md 执行 12 条 Git 强约束。
- 标记：P0。
