...
## 用户域
- 最新小切片：DDD-USER-ORDER-QUERY-DETAIL-DATASCOPE-POLICY 已将 `OrderQueryService.getOrderDetail` 详情访问范围判断按灰度双路径收口：`ddd.refactor.data-scope-policy.enabled=false` 默认关闭时保留 Legacy `assertCanAccessLegacy`，开启后通过用户域 `DataScopePolicy.contextRequirement` / `decide` 判定 PERSONAL / DEPT，缺少 user/dept 上下文仍拒绝。本轮不改订单事实、归因、金额、接口参数、默认开关或真实数据。报告：`harness/reports/evidence-20260622-141704.md`。风险变化：`OrderQueryServiceTest` 覆盖源码边界、默认关闭拒绝语义、灰度开启拒绝语义；定向组合 71 tests PASS，`agent-do` backend PASS。
...
- 当前补充：`OrderAttributionService`、`PerformanceMetricsQueryService`、`DashboardService` 与 `DataApplicationService` 已委托 `DataScopePolicy` 处理数据范围过滤；`OrderService` 与 `OrderQueryService` 保留默认关闭 Legacy 路径，灰度开启后消费用户域 `DataScopePolicy`。
