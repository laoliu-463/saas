...
## 用户域
- 最新小切片：DDD-USER-SAMPLE-PAGE-DATASCOPE-POLICY 已将 `SampleApplicationService.getSamplePage` 寄样列表 PERSONAL auditor 查询判断按灰度双路径收口：`ddd.refactor.data-scope-policy.enabled=false` 默认关闭时保留 Legacy `DataScope.PERSONAL + plain biz staff` 路径，开启后通过用户域 `DataScopePolicy.contextRequirement` / `decide` 解释数据范围，再由寄样列表读服务选择 `findPageForAuditor` 或 `findPageWithScope`。本轮不改寄样状态机、列表筛选参数、Mapper SQL、VO 组装、接口契约、默认开关或真实数据；PERSONAL 缺少 userId 时继续走 Legacy auditor 查询，避免灰度开启后扩大可见范围。报告：`harness/reports/evidence-20260622-173855.md`；retro：`harness/reports/retro-20260622-173923.md`。风险变化：targeted/组合测试 PASS，后端全量 `mvn test` PASS，`agent-do` backend PASS，real-pre preflight PASS。
- 上一小切片：DDD-USER-SAMPLE-FILTER-OPTIONS-DATASCOPE-POLICY 已将 `SampleFilterOptionsService` 寄样筛选动态选项加载的招商专员 PERSONAL auditor 查询判断按灰度双路径收口：`ddd.refactor.data-scope-policy.enabled=false` 默认关闭时保留 Legacy `DataScope.PERSONAL + plain biz staff` 路径，开启后通过用户域 `DataScopePolicy.contextRequirement` / `decide` 解释数据范围，再由寄样读服务选择 `findPageForAuditor` 或 `findPageWithScope`。本轮不改寄样状态机、筛选项含义、Mapper SQL、接口参数、默认开关或真实数据；PERSONAL 缺少 userId 时继续走 Legacy auditor 查询，避免灰度开启后扩大可见范围。报告：`harness/reports/evidence-20260622-172132.md`；retro：`harness/reports/retro-20260622-172157.md`。风险变化：targeted/组合测试 PASS，后端全量 `mvn test` PASS，`agent-do` backend PASS。
