...
## 用户域
- 最新小切片：DDD-USER-AUTH-DATASCOPE-PERMISSION-POLICY 已将 `AuthService.login` / `refreshToken` 的登录上下文 dataScope 计算委托给用户域 `CurrentUserPermissionPolicy.resolveDataScopeCode`，不再在认证服务中本地 `roleCodes.contains(admin/ops_staff)` 判断。本轮不改用户名解析、密码校验、失败锁定、JWT 字段、refresh 吊销、登录/登出审计或真实账号数据。报告：`harness/reports/evidence-20260622-121825.md`。风险变化：`AuthServiceTest` 覆盖 `" OPS_STAFF "` 与 `" ADMIN "` 归一化后 dataScope=3，`DddUserAuthDataScopePolicyBoundaryTest` 防止认证服务重新引入本地角色 matcher。
- 上一小切片：DDD-USER-MASTER-DATA-PERMISSION-POLICY 已将 `UserMasterDataService` 的主数据角色编码归一、下拉角色过滤和跨部门管理员判断委托给用户域 `CurrentUserPermissionPolicy.normalizeRoleCodes` / `hasAnyRole`。本轮不改 `channels` / `recruiters` / `group-members` 查询 Mapper、active 用户过滤、关键词、limit、排序或真实数据。结论报告：`harness/reports/evidence-20260622-120401.md`；中间报告 `harness/reports/evidence-20260622-105205.md` 仅记录 `agent-do` 在 build/restart/health 后业务验证前超时的 FAIL 事实。
