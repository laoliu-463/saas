...
> **主源说明**：本文件的"验证入口表"与 `harness/rules/governance/task-routing.md` 的"任务类型分流表"互为对偶：
> - 本文件：列**命令**与适用环境。
> - `TASK_ROUTING.md`：列**任务类型**与必读文件。
> 详细 scope → command 决策以 `harness/rules/runbooks/governance/scope-command-matrix.md` 为权威表。
