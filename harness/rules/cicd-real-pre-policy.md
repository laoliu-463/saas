...
→ Jenkins 全局队列 → 按差异决定是否迁移 → digest 固定部署 → 适用版本验证 → 发布记录
```
...
Jenkins 在全局锁内比较 `current.json` 的当前 SHA 与目标 SHA。只有以下情况将 `databaseMigration.required` 记为 `true` 并执行远端迁移：

- 首次发布，没有可比较的当前 SHA；
- `backend/src/main/resources/db/` 或 `scripts/run-real-pre-db-migrations.sh` 发生变化。

纯 Harness、文档、前后端普通代码或镜像重建不触发数据库写操作，清单记录 `required=false`、原因和差异路径。经批准的应用回滚遵循数据库 forward-only 原则，不自动执行旧迁移或逆向数据库。
...
后端仍返回 `imageDigest`、`databaseMigrationVersion` 和 `flywayVersion` 供只读留证。当 `required=true` 时，数据库与 Flyway 版本必须和发布清单严格一致，`UNAVAILABLE`、`NOT_MANAGED`、缺字段或不一致均失败；当 `required=false` 时只记录观测值，不要求执行迁移，也不因 Flyway 未启用单独阻断应用发布。

当前 `.env.real-pre.example` 将 `FLYWAY_ENABLED=false` 作为安全默认值。包含数据库迁移的发布须先完成迁移演练并由服务器配置负责人显式启用；无数据库变更的发布不得借此强制迁移。
