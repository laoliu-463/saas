...
2. `CLAUDE.md`
3. `docs/README.md`
4. `harness/rules/state/snapshots/01-当前项目状态.md`
5. `harness/rules/governance/task-routing.md`
...
- 先读 `harness/rules/runbooks/ddd/DDD_OPTIMIZATION_ROADMAP.md` 和 `harness/rules/runbooks/ddd/DDD_DOMAIN_TASK_MATRIX.md`。
- 再读对应 `harness/rules/instructions/domain/*.md` 与 `docs/领域/*.md`。
- 修改前必须确认 `harness/rules/governance/forbidden-scope.md`，不得扩大当前范围。
...
后续 Agent 不允许临时发明构建、重启、部署流程。除非用户明确要求 `test`，默认使用本地 `real-pre`；远端部署仍必须由用户明确要求后再传 `-DeployRemote true`。除非存在已登记的安全阻塞，否则必须优先调用 `harness/scripts/commands/agent-do.ps1` 或其中的子脚本。
...
Gate 定义、分类和验证要求见 `harness/rules/governance/COMPLETION_GATES.md`。Agent 必须在任务开始时声明本次选择的 Gate，执行中发现影响范围扩大时必须升级 Gate，不能降级。

## Session Exit Gate

所有 Agent 会话结束前必须执行 `harness/rules/governance/session-exit-gate.md`。
...
任何 Agent 任务必须按 `harness/rules/skills/git/git-change-control.md` 的 Gate 顺序执行：
...
详细规则、命令模板、报告模板见 `harness/rules/skills/git/git-change-control.md` 和 `harness/rules/skills/git/git-batch-submit.md`。
