...
> 任务 ID：GITHUB-COLLABORATION-PHASE1-166
> 更新时间：2026-07-19
...
### v0.8.3 — 2026-07-19
- 建立 Issue → 独立 worktree/分支 → Draft PR → CI → Merge Queue 的 GitHub 协作合同，普通任务不再拥有直接合并或部署权限。
...
- 代码已合并至 `feature/auth-system` 并部署远端 `real-pre`：运行源码和前后端镜像为 `672baed6`，PostgreSQL、Redis、后端和前端均健康。
