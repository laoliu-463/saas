...
> 本文件定义按文件路径自动判定任务风险等级（Risk）的规则。配置由 `harness/rules/test-impact-map.json` 承载，本文档说明判定语义和优先级。
...
| R3 | DB、权限、角色、订单归因、业绩金额、状态机、定时任务、外部写接口、CI/CD、Harness 规则 | `backend/src/main/resources/db/**`、`**/rbac/**`、`**/security/**`、`Jenkinsfile`、`harness/rules/**` |

> **关键**：`AGENTS.md` / `CLAUDE.md` / `CONTRIBUTING.md` / `harness/rules/**` / `Jenkinsfile` / `.github/workflows/**` 属于"会改变工程行为的文档"，**不是 R0，按 R3 处理**。
...
完整规则列表见 `harness/rules/test-impact-map.json` §rules。当前共 12 条：
