...
| RISK-011 | 分析模块 | ~~P0~~→P2 | ~~V1 不做毛利但前端仍展示 grossProfit~~ **已撤销**：2026-06-05 用户决策毛利纳入 V1，降级为前端展示补齐任务 | MetricsVO + data/index.vue 未做 V1 裁剪 | **REVOKED** | `harness/reports/dashboard-money-audit-001-20260604-131908.md` | GROSS-PROFIT-DISPLAY-001 前端补齐毛利展示 | 否（已撤销 P0 状态） |
...
| RISK-013 | 订单域 / 业绩域 | P0 | 订单已落库但部分订单缺 `performance_records` | 已验证为 `FAIL_EVENT_BEFORE_COMMIT + MISSING_BACKFILL`：订单事务提交前发布异步事件，Listener 查询不到订单后跳过；本地代码已改为 after-commit 发布，但历史缺口仍存在 | FIXED_CODE_PENDING_BACKFILL | `harness/reports/order-performance-event-after-commit-fix-001-20260606-121000.md`、`harness/reports/evidence-20260606-120829.md` | `ORDER-PERFORMANCE-BACKFILL-001` 使用显式 backfill 入口补齐历史缺口并 anti-join 清零 | 是，阻塞订单→业绩历史完整性 |
