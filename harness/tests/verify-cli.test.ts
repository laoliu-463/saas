...
import { createCheckResult, createRunResult, type CheckStatus } from "../src/core/result.js";
import {
...
describe("verify CLI", () => {
  ...
  it("只消费 agent-do 注入的合法 Git 快照，并用不可猜调用标识输出唯一回执", async () => {
    ...
    const snapshot = {
      ...
      branch: "codex/task10",
      clean: false,
      changedFiles: ["harness/src/cli/verify.ts"],
      identity: {
        kind: "WORKTREE",
        headSha: "a".repeat(40),
        changedFiles: ["harness/src/cli/verify.ts"],
  ...
  it("CLI 与组合器源码不包含 Git 写操作、SSH 或部署调用", () => {
    const source = [
      readFileSync(new URL("../src/cli/verify.ts", import.meta.url), "utf8"),
      readFileSync(new URL("../src/workflows/run-verify.ts", import.meta.url), "utf8"),
    ].join("\n");
