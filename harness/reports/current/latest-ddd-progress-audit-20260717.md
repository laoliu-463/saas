...
| 域 | 状态 | 真实依赖与证据 | 未达成原因 |
|---|---|---|---|
| user | ROUTED | user application/port/adapter 已被权限与用户流程使用 | Legacy auth/user service 仍存在；本轮仅有管理员与部分 RBAC 运行证据 |
| product | ROUTED | product application、查询/策略和活动同步旁路存在；商品链 real-pre 31 通过 | ProductService/旧 Mapper 仍是部分最终路径，未证明新链为全量主路径 |
| order | ROUTED | order application/read facade 与订单事件存在；订单读取、同步和归因仍依赖旧服务 | 订单同步链仍由 OrderSyncService 等旧实现承担，未 RETIRED |
...
本次没有任何域可标记 `PRIMARY`、`VERIFIED` 或 `RETIRED`：本地 real-pre 商品链和 dashboard 通过不等于 DDD 新链完成真实对账；订单归因、寄样链和多角色权限仍有 `PENDING/FAIL/BLOCKED`。
...
红线：后续只能下降不能增长；禁止新增 Mapper 或跨域职责。本轮没有大规模拆解，只收敛 Schema 守卫与迁移入口。

## 综合真实完成度

按 `SHELL=0`、`SHADOW=0.25`、`ROUTED=0.5`、`PRIMARY=0.75`、`VERIFIED/RETIRED=1` 的保守权重，10 个业务域为约 **38%**。这是“真实主链迁移并有证据”的完成度，不是目录或类数量完成率；下一切片应优先选择低风险、可回退的 config 或 analytics 旁路，并先补 real-pre 角色凭证与订单样本。
