...
ERROR OrderSyncJob  syncInstituteOrdersHotRecent failed
      ...
      endpoint=buyin.instituteOrderColonel, msg=认证失败, sign校验失败
```
...
→ The signature rejection is **global to all Douyin endpoints**, not specific to order sync.

## E.9 Read-only SQL queries

All queries are SELECT-only. No INSERT/UPDATE/DELETE/DDL.

### E.9.1 Q1 — Freshness lag (CST-aware)
...
### E.9.3 Q3 — Redis checkpoints (read via `docker exec` with `REDIS_PASSWORD` from `.env.real-pre`)
...
| `order:sync:circuit_breaker:buyin_institute_order_colonel` | (nil) — closed | — |
| `order:sync:circuit_breaker:buyin_colonel_multi_settlement_orders` | (nil) — closed | — |
| `order:sync:institute_hot_lock` TTL | -2 (not held) | — |
| `order:sync:institute_recent_lock` TTL | -2 (not held) | — |
...
### E.9.4 Q4 — `pay_time` 5-min histogram (last 2 h, CST)
...
### E.9.5 Q5 — `update_time` lag distribution (last 200 rows in past 2 h)
...
SLA target is p95 ≤ 120 s. **Actual p95 = 1599 s — SLA is violated by ~13×**, and 0/200 rows are within 120 s.
...
- Any DDL / migration
- Frontend
- `.env.real-pre` (`DOUYIN_CLIENT_SECRET` untouched — not a config rotation task)
