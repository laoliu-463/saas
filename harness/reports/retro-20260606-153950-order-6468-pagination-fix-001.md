...
- Time: 2026-06-06 15:39:50 +08:00
...
- Snapshot at: 2026-06-06T15:39:50+08

## 2. Repeated probing

- DB 抽样脚本运行 4 次（counts/duplicates/anti-join/15:14→15:39 增量）— 无重试
- curl /api/dashboard/summary 与 /metrics 各 3 次（修复前 + 15:14 + 15:39）— 无 5xx
- syncItems 调度触发 2 次（real-pre 走真上游授权，未 dry-run 短路）
...
- DB 抽样（counts、duplicates、anti-join、pay_time 范围、15:14→15:39 增量）
- 同步响应 JSON（首屏 dry-run 与真实 121 轮 cursor 翻页对比）
- /api/dashboard/summary ×3
- /api/dashboard/metrics ×3
- 调度日志 ORDER_SYNC_INSTITUTE 完整字段（pagesFetched / uniqueOrders / inserted / updated / failed / stopReason）
- 资源占用（heap、连接池、Redis 锁）
- 修复后实时观察（15:14→15:39 = +796 单）

未做的验证（已明确标记为后续）：

- 6/5 ~ 6/6 真实上游订单数（抖音控制台核对）— 留作运营 review
- 商品域转链后归因率重测 — 留作下游
- **bizCommission 15:14→15:39 下跌 19.85 的根因** — 已生成排查 SQL，留作下一步
- 服务费支出（serviceFeeExpense）= 1.90 的来源字段（推广/工具/通道分润）— 待与商品域对齐
...
- 当前 9 指标定义中 talentCommission 已不在 UI 暴露（无二级团长），但 `PerformanceMetricsQueryService` 仍计算该字段 — 建议下次清理时同步删字段
...
不需要新脚本。本次修复用到的脚本都是 `before.sh` / `after.sh` / `sync-trigger.sh` / `snap-current.sh` 等既有文件。
...
- 下次若再做大批量同步（>10 000 单），建议先在 Mongo 风格短时缓存中保留 cursor 状态，
  避免进程崩溃后从首页重拉。本期单轮 1m 28s 内完成，不影响 SLA。
- 7 日趋势出现"6/5 +6/6 大幅上涨"需要业务侧在群内同步给运营团队，
  避免被误判为"重复刷量"。
- **bizCommission 异常下降需在下次拉数据前排查**：跑 §4 待查 SQL 确认
  15:14→15:39 新增 796 单中是否出现 `biz_recruiter_id` 被重写为 NULL 的情况。
- **服务费支出 1.90 字段归属待对齐**：与商品域确认此字段是否应纳入 V1 9 指标，还是
  单独走"运营成本"轨道。
...
| 8 | 必须记录 snapshotAt | ✅ | 2026-06-06T15:39:50+08 三处一致 |
