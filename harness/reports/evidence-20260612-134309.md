# Evidence Report

## Metadata

- Time: 2026-06-12 13:43:09 +08:00
- Environment: real-pre
- Scope: backend
- Branch: feature/ddd/DDD-SAMPLE-005-FIX-sample-agent
- Commit: 87fa5af5
- Worktree: dirty
- Deploy remote: false

## Modified Files

~~~text
backend/src/main/java/com/colonel/saas/constant/OrderDomainEventTypes.java
backend/src/main/java/com/colonel/saas/domain/event/OutboxEventAppender.java
backend/src/main/java/com/colonel/saas/domain/order/event/OrderDomainEventOutboxRouter.java
backend/src/main/java/com/colonel/saas/domain/order/event/OrderDomainEventPublisher.java
backend/src/main/java/com/colonel/saas/job/DomainEventDispatcherJob.java
backend/src/main/java/com/colonel/saas/service/OrderSyncPersistenceService.java
backend/src/test/java/com/colonel/saas/architecture/DddOutbox001OrderRoutingTest.java
backend/src/test/java/com/colonel/saas/job/DomainEventDispatcherJobTest.java
backend/src/test/java/com/colonel/saas/service/OrderSyncPersistenceInstituteSettlementTest.java
backend/src/test/java/com/colonel/saas/service/OrderSyncPersistenceServiceTest.java
harness/reports/content-retire-20260612-134309.md
harness/reports/ddd-outbox-001-order-synced-20260612.md
harness/reports/retro-20260612-133336.md
harness/tasks/ddd-multi-agent-board.md
~~~

## Git Status

~~~text
 M backend/src/main/java/com/colonel/saas/domain/event/OutboxEventAppender.java
 M backend/src/main/java/com/colonel/saas/job/DomainEventDispatcherJob.java
 M backend/src/main/java/com/colonel/saas/service/OrderSyncPersistenceService.java
 M backend/src/test/java/com/colonel/saas/job/DomainEventDispatcherJobTest.java
 M backend/src/test/java/com/colonel/saas/service/OrderSyncPersistenceInstituteSettlementTest.java
 M backend/src/test/java/com/colonel/saas/service/OrderSyncPersistenceServiceTest.java
 M harness/tasks/ddd-multi-agent-board.md
?? backend/src/main/java/com/colonel/saas/constant/OrderDomainEventTypes.java
?? backend/src/main/java/com/colonel/saas/domain/order/event/OrderDomainEventOutboxRouter.java
?? backend/src/main/java/com/colonel/saas/domain/order/event/OrderDomainEventPublisher.java
?? backend/src/test/java/com/colonel/saas/architecture/DddOutbox001OrderRoutingTest.java
?? harness/reports/content-retire-20260612-134309.md
?? harness/reports/ddd-outbox-001-order-synced-20260612.md
?? harness/reports/retro-20260612-133336.md
~~~

## Build Result

~~~text
not collected
Backend build: PASS (mvn -f backend/pom.xml -DskipTests package)
~~~

## Docker Status

### docker compose ps

~~~text
NAME                              IMAGE                            COMMAND                   SERVICE             CREATED             STATUS                       PORTS
saas-active-backend-real-pre-1    colonel-saas/backend:real-pre    "sh -c 'java $JAVA_O鈥?   backend-real-pre    19 seconds ago      Up 17 seconds (healthy)      127.0.0.1:8081->8080/tcp
saas-active-frontend-real-pre-1   colonel-saas/frontend:real-pre   "/docker-entrypoint.鈥?   frontend-real-pre   About an hour ago   Up About an hour (healthy)   127.0.0.1:3001->80/tcp
saas-active-postgres-real-pre-1   postgres:15-alpine               "docker-entrypoint.s鈥?   postgres-real-pre   41 hours ago        Up About an hour (healthy)   5432/tcp
saas-active-redis-real-pre-1      redis:7-alpine                   "docker-entrypoint.s鈥?   redis-real-pre      6 days ago          Up About an hour (healthy)   6379/tcp
~~~

### docker ps

~~~text
NAMES                             STATUS                       PORTS
saas-active-backend-real-pre-1    Up 17 seconds (healthy)      127.0.0.1:8081->8080/tcp
saas-active-frontend-real-pre-1   Up About an hour (healthy)   127.0.0.1:3001->80/tcp
saas-active-postgres-real-pre-1   Up About an hour (healthy)   5432/tcp
saas-active-redis-real-pre-1      Up About an hour (healthy)   6379/tcp
saas-test-backend-1               Up About an hour (healthy)   0.0.0.0:5005->5005/tcp, [::]:5005->5005/tcp, 0.0.0.0:8080->8080/tcp, [::]:8080->8080/tcp
saas-test-postgres-1              Up About an hour (healthy)   0.0.0.0:5432->5432/tcp, [::]:5432->5432/tcp
saas-test-redis-1                 Up About an hour (healthy)   6379/tcp
~~~

## Health Check Result

~~~text
Local health verification: PASS
~~~

## Business Validation Result

~~~text
Business validation: PASS (npm run e2e:real-pre:p0:preflight)
~~~

## Content Maintenance Result

~~~text
Content maintenance: Plan. Manifest=. DryRun=False.
~~~

## Remote Deploy Result

~~~text
remote not deployed
~~~

## Conclusion

PASS

## Residual Risk

- Items marked as not collected are not proof of success.
- If real-pre lacks real orders or pick_source samples, record the result as PENDING or PARTIAL.
