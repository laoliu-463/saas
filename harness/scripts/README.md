...
验证、证据、Git 和发布命令彼此分离；`agent-do.ps1` 只负责本地执行与证据，不自动 commit、push 或部署。`ReportKey` 必须稳定，`OwnedFiles` 使用分号分隔仓库相对路径。

| 命令 | 用途 | 适用环境 | 是否安全 | 是否改数据 | real-pre 是否允许 | 证据输出 |
| --- | --- | --- | --- | --- | --- | --- |
| `agent-do.ps1 -Env real-pre -Scope docs -ReportKey task-key -OwnedFiles 'path1;path2' -Message "docs: update harness"` | docs-only 本地验证与证据 | real-pre | 安全检查后执行 | 不改业务数据 | 可用于 docs scope | `reports/current/latest-task-key.md` |
...
| `harness/scripts/commands/inspect.ps1` | 只读查看分支、commit、dirty 和变更文件 | local | 安全 | 不改数据 | 允许 | 控制台输出 |
| `harness/scripts/commands/verify.ps1` | 独立执行本地健康验证 | test / real-pre | 安全 | 不改数据 | 允许 | HTTP 响应 |
| `harness/scripts/commands/evidence.ps1` | 独立生成稳定 evidence | test / real-pre | 安全 | 写报告文件 | 允许 | `harness/reports/current/latest-<key>.md` |
| `harness/scripts/commands/release-verify.ps1` | 只读校验 release manifest、main 祖先关系和无代码漂移 | local | 安全 | 不改数据 | 允许 | 控制台输出 |
| `harness/scripts/commands/git-commit.ps1` | 按 OwnedFiles 显式暂存并 commit，不 push | local | 会改 Git 状态 | 不改业务数据 | 允许 | Git 输出 |
| `harness/scripts/commands/git-push.ps1` | 推送已经显式创建的 commit | local | 会改远端 Git | 不改业务数据 | 需明确执行 | Git 输出 |
...
| `harness/scripts/commands/git-push-safe.ps1` | 兼容旧入口；新任务使用 `git-commit.ps1` + `git-push.ps1` 分开执行 | 本地 | 会改 Git 状态 | 不改业务数据 | 不涉及环境 | Git 输出 |
