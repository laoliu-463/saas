...
| --- | --- | --- | --- | --- | --- | --- |
| `powershell -NoProfile -ExecutionPolicy Bypass -File .\harness\scripts\commands\agent-do.ps1 -Env real-pre -Scope docs -Message "docs: update harness"` | docs-only 总入口 | real-pre | 安全检查后执行 | 不改业务数据 | 可用于 docs scope | `harness/reports/evidence-*.md` |
| `powershell -NoProfile -ExecutionPolicy Bypass -File .\harness\scripts\commands\agent-do.ps1 -Env real-pre -Scope backend -Message "fix: backend"` | 后端构建、重启、健康检查、验证 | real-pre | 会重启服务 | 不应清库；验证以 preflight 为默认 | 默认允许，但必须通过 real-pre 安全检查 | evidence report |
| `powershell -NoProfile -ExecutionPolicy Bypass -File .\harness\scripts\commands\agent-do.ps1 -Env real-pre -Scope frontend -Message "fix: frontend"` | 前端构建、重启、健康检查、验证 | real-pre | 会重启服务 | 不改业务数据 | 默认允许，但必须通过 real-pre 安全检查 | evidence report |
| `powershell -NoProfile -ExecutionPolicy Bypass -File .\harness\scripts\commands\agent-do.ps1 -Env real-pre -Scope full -Message "fix: real-pre"` | real-pre 全链路本地验证 | real-pre | 高风险，需安全检查 | 默认 preflight 只读为主；专项 E2E 可能产生真实上游副作用 | 默认允许，但远端部署仍需用户明确要求 | evidence report、runtime/qa/out |
| `powershell -NoProfile -ExecutionPolicy Bypass -File .\harness\scripts\commands\agent-do.ps1 -Env real-pre -Scope apifox -Message "chore: apifox harness"` | Apifox/OpenAPI 本地 harness | real-pre | 安全检查后执行 | 不改业务数据，不导入云端 | 允许；跳过业务容器重启 | evidence report |
...
| `harness/scripts/commands/collect-evidence.ps1` | 生成 evidence report | test / real-pre | 安全 | 写报告文件 | 允许 | `harness/reports/evidence-*.md` |
| `harness/scripts/commands/retire-content.ps1` | 旧内容维护计划、按 manifest 归档或删除 | local | Plan 安全；Archive/Delete 有风险 | Archive/Delete 会移动或删除文件 | real-pre 只影响仓库文件，不碰数据；删除需 manifest | `harness/reports/content-retire-*.md` |
| `harness/scripts/commands/new-retro.ps1` | 生成复盘报告 | test / real-pre | 安全 | 写报告文件 | 允许 | `harness/reports/retro-*.md` |
| `harness/scripts/commands/deploy-remote.ps1` | 远端 real-pre 部署 | real-pre | 高风险 | 会更新远端服务 | 仅用户明确要求时允许 | 远端 docker ps、健康检查 |
| `harness/scripts/commands/git-push-safe.ps1` | 敏感文件检查、commit、push | 本地 | 会改 Git 状态 | 不改业务数据 | 不涉及环境 | Git 输出 |
