...
export interface VerifyCliDependencies {
  ...
  readSnapshotFile?: (path: string) => string;
...
const DEFAULT_DEPENDENCIES: VerifyCliDependencies = {
  ...
  readSnapshotFile: (path) => readFileSync(path, "utf8"),
...
function readInjectedGitSnapshot(dependencies: VerifyCliDependencies): string | undefined {
  const snapshotFile = dependencies.readEnvironmentVariable("HARNESS_VERIFY_GIT_SNAPSHOT_FILE");
  if (snapshotFile !== undefined && snapshotFile.trim().length > 0) {
    try {
      return (dependencies.readSnapshotFile ?? ((path: string) => readFileSync(path, "utf8")))(snapshotFile);
    } catch {
      throw new VerifyContractError("agent-do 注入的 Git 快照文件不可读。");
    }
  }
  return dependencies.readEnvironmentVariable("HARNESS_VERIFY_GIT_SNAPSHOT_JSON");
}
...
export async function verifyCli(
  ...
  try {
    const injectedSnapshot = parseInjectedGitSnapshot(
      readInjectedGitSnapshot(dependencies),
    );
