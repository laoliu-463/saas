...
import {
  ...
  writeEvidence,
} from "../../lib/node/core/evidence.js";
import { createRunContext } from "../../lib/node/core/run-context.js";
import { createCheckResult, createRunResult } from "../../lib/node/core/result.js";
import type { GitSnapshot } from "../../lib/node/core/git.js";
...
describe("结构化证据", () => {
  ...
  it("evidence Schema 直接引用 RunResult 而不重复状态定义", () => {
    const evidenceSchema = JSON.parse(
      readFileSync(
        new URL("../../lib/contracts/evidence-report.schema.json", import.meta.url),
        "utf8",
      ),
  ...
  it("顶层运行摘要保持 JSON 原值但在 Markdown 中按安全段落展示", async () => {
    ...
    const module = await import("../../lib/node/core/evidence.js");
  ...
  describe("canonical 路径边界", () => {
    ...
    for (const target of ["runtime"] as const) {
      it(
        `${target} 证据父目录链接到仓库外时写前拒绝且外部目录无文件`,
        (context) => {
          const root = makeRoot();
          const external = makeRoot();
          const link = join(root, "runtime", "qa", "out");
