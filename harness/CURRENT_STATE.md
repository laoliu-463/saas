...
- 重要口径：本次治理针对 SYNC-PLAN-001 暴露的"多任务 dirty 膨胀 + 提交门禁缺失"问题。Batch 3 提交时已使用 Dirty Classification 和 B3-SCOPE-001 范围隔离作为本任务新规的事前验证，证明强约束可执行。下一步：执行 Batch 4 报告提交与 Batch 5 cleanup-retire 提交（按 SYNC-PLAN-001 推荐顺序），同时对 P-FIX-002 配置残留（`application-real-pre.yml`）单独任务收口。
