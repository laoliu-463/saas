...
- Playwright 最小复现：DOM 首屏商品 ID 与页面 Network `/api/products?page=1&size=5` 完全一致。
- 页面首屏商品 ID：`3817202813663051896`、`3820974207190302840`、`3787558242947826069`、`3781068948766851546`、`3810562766247428542`。
- 同时出现活动：`3916506`、`3929905`，说明 `/product/manage/products` 首屏是全局商品推进池，不是 3916506 单活动列表。
- 3916506 活动商品接口 `status=1,count=5` 返回顺序：`3810562766247428542`、`3781068948766851546`、`3787558242947826069`、`3817202813663051896`、`3819313446558761397`。
- 对页面首屏 5 个商品逐个按商品 ID 回查各自活动商品接口，标题与 `status/statusText` 一致；未发现同一商品字段在页面与接口之间不一致。
- 阶段性判断：当前复现到的是“页面全局排序/跨活动范围”与“单活动接口排序/范围”不一致，不是同一商品详情字段渲染错误。
