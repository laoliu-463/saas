...
if ([string]::IsNullOrWhiteSpace($adminPassword)) { throw "Set SAAS_QA_ADMIN_PASSWORD before running qa-env.ps1." }
...
(async () => {
  ...
  await page.getByTestId("login-password").locator("input").fill(process.env.SAAS_QA_ADMIN_PASSWORD || "");
