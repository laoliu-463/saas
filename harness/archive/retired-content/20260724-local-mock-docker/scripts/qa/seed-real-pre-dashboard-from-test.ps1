...
$adminPassword = $env:SAAS_QA_ADMIN_PASSWORD
if ([string]::IsNullOrWhiteSpace($adminPassword)) { throw "Set SAAS_QA_ADMIN_PASSWORD before running seed-real-pre-dashboard-from-test.ps1." }
...
try {
    $login = Invoke-RestMethod -Method POST -Uri "http://localhost:8081/api/auth/login" `
        -ContentType "application/json" -Body (@{ username = "admin"; password = $adminPassword } | ConvertTo-Json)
