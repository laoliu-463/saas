...
- Time: 2026-06-19 17:36:50 +08:00
...
DDD-BASELINE-001 + DDD-USER U-5: restored backend testCompile baseline, fixed stale target/classes evidence path, then made SysUserService dataScope branch decision delegate to DataScopePolicy.decide while preserving SQL output shape. agent-do not used because it calls git-push-safe in a broad dirty worktree; manual backend package, restart-compose backend, and verify-local backend health were executed instead.
