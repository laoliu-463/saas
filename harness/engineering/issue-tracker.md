...
- **harness 索引**：`harness/engineering/issues-index.md` 维护 issue 列表镜像（每周自动更新或手动 sync）
...
- 若仓库后续改用 Jira、Linear 或本地 markdown issue，**更新本文件 + harness/engineering/issues-index.md**，而不是让 skill 自行猜测。
...
- `harness/engineering/triage-labels.md` —— 5 个 canonical 标签映射
- `harness/engineering/context.md` —— 上下文文档消费规则
- `harness/engineering/issues-index.md` —— GitHub Issues 的本地镜像（待建立）
- `docs/决策/ADR-*.md` —— 架构决策
- `CONTEXT.md`（根）—— 项目领域词汇
...
- **v2.0**（2026-06-19）：迁移到 `harness/engineering/issue-tracker.md`，改为 GitHub + harness 索引混合模式