...
export function renderVerifyHelp(): string {
  return [
    ...
    "",
    "用法：",
    "  npm run harness:verify -- --env test --scope backend --report-key task-key",
    "  npm run harness:verify -- --env real-pre --scope full --report-key task-key [--business-command \"npm run e2e:real-pre:p0:preflight\"]",
    "",
    "参数：",
