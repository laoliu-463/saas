...
function Get-AutoCandidates {
    ...
    $debtRegister = if ($debtRegisterItem) { $debtRegisterItem.FullName } else { "" }
    if (Test-Path -LiteralPath $debtRegister) {
        $content = Get-Content -Raw -Encoding UTF8 -LiteralPath $debtRegister
