...
[V1 必做] 本文件只做仓库地图，不做百科全书。业务事实、领域合同、流程、接口、验收和部署口径均进入 `docs/`。

[V1 必做] 智能体工作台统一放在 `.claude/`，包括 hooks、skills、plugins、LSP、MCP、subagents、commands、memory、qa 和 templates。

[历史归档] 旧 V2.2 完整方案、旧领域设计、local-mock 旧口径、FastAPI / Celery / Python 爬虫式设想只作为背景，不能写成当前事实。

## 当前事实

[V1 必做] 当前技术栈以 Spring Boot、PostgreSQL、Redis、Docker Compose、Vue 3 / TypeScript、Playwright 为准。
...
[V1 简化] 旧文档需要查证时，从 `docs/归档/旧版V2.2完整方案.md` 进入，不直接把旧口径带回主线。
...
[V1 必做] 任何修改前先确认所属领域、流程和验收项；冲突写入 `docs/决策/ADR-002-V1范围优先级.md`，不要自行拍板。

[V1 必做] 修改代码时必须同步对应文档；本轮只允许文档和 `.claude/` 工程化文档变更。

[V1 必做] 验收必须能落到脚本、SQL/API、证据路径三类之一。没有证据时只能写阶段性结论。

[V2 预留] 完整 V2.2 能力、独家达人 / 商家全量闭环、完整重算、MQ 化事件、外部 `quick_sample_apply`、真实物流自动化均按后续专项处理。

## 常用入口

[V1 必做] `npm run e2e:v1-p0`：test/mock V1-P0 基线。

[V1 必做] `npm run e2e:real-pre:p0`：real-pre 统一验收入口。

[V1 必做] `npm run e2e:real-pre:p0:preflight`：real-pre 预检。
