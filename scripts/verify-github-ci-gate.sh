...
#   github-ci-jobs.tsv          one row per job (name, status, conclusion)
# ============================================================================
set -eu
set -o pipefail
...
# without conflicting with another heredoc. (The original PR #198 had two
# heredocs on one command, which is invalid bash and broke syntax check.)
TMPDIR_FOR_GATE="${TMPDIR:-/tmp}"
RUNS_PARSER="$(mktemp "${TMPDIR_FOR_GATE}/verify-github-ci-gate.runs.XXXXXX.py")"
JOBS_PARSER="$(mktemp "${TMPDIR_FOR_GATE}/verify-github-ci-gate.jobs.XXXXXX.py")"
trap 'rm -f "$RUNS_PARSER" "$JOBS_PARSER"' EXIT
...
while [ "$ELAPSED" -lt "$WAIT_LIMIT_SECONDS" ]; do
    ...
    PARSED=$(printf '%s' "$BODY" | python3 "$RUNS_PARSER" "$GITHUB_SHA") || PARSED=""
...
# Write a TSV of job name / status / conclusion for postmortem.
printf '%s' "$JOBS_BODY" | python3 -c "
import json, sys
data = json.loads(sys.stdin.read())
for j in data.get('jobs', []):
...
for REQUIRED in "${REQUIRED_JOBS[@]}"; do
    LINE=$(printf '%s' "$JOBS_BODY" | python3 "$JOBS_PARSER" "$REQUIRED") || LINE=""
    if [ -z "$LINE" ]; then
