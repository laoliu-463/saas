...
function Get-LatestReportPath {
    ...
    $dir = Get-ChildItem -LiteralPath $OutRoot -Directory |
        Where-Object { $_.Name -like 'e2e-data-gap4-visible-*' } |
...
$qaScript = Join-Path $repoRoot 'runtime\qa\data-gap4-visible.cjs'
...
Assert-HttpOk -Name 'test frontend' -Url 'http://localhost:3000'
Assert-HttpOk -Name 'test backend health' -Url 'http://localhost:8080/api/system/health'
...
if ($null -eq $reportPath -or -not (Test-Path -LiteralPath $reportPath)) {
    throw "data-gap4 visible QA completed, but report.md was not found."
}

Write-Host ""
Write-Host "data-gap4 visible QA completed." -ForegroundColor Green
