...
<<<<<<< HEAD
                ...
                # release/real-pre may carry release-controller, operator
                # documentation, and QA E2E updates without rebuilding
                # application images. Keep those paths explicit; all runtime
                # code/configuration must still match sourceMainSha exactly.
...
>>>>>>> 643e04cb (fix: run database migrations only when required)
        }
