...
- Create: `Git history/reports-root-retirement-20260713.json` — 75 个已跟踪时间戳报告清单。
- Modify: `AGENTS.md`、`harness/README.md`、`harness/README.md`、`harness/scripts/README.md` 和相关 canonical rules/templates — 新入口和状态口径。
...
结果对象固定包含 `TaskGate`、`RepositoryHealth`、`Violations`、`Warnings`、`HistoricalDebt`。任务新增/恶化硬违规退出 1；仅历史债务退出 0，并将报告稳定写入 `runtime/qa/out/latest-harness-limits-check.md`。
...
(New-HarnessReportPath -RepoRoot $repo -ReportKey 'file-governance') |
  Should Be (Join-Path $repo 'runtime\qa\out\latest-file-governance.md')
...
```json
{"path":"runtime/qa/out/evidence-20260713-000000.md","action":"archive","archiveGroup":"evidence"}
```
...
manifest 只包含 `git ls-tree HEAD` 中 `runtime/qa/out/` 根的 75 个 `evidence-*`、`retro-*`、`content-retire-*`；分别使用三个 group。执行 Archive dry-run，Expected: 75 条操作、0 个缺失、每组不超过 50。

- [x] **Step 4: 执行归档并复查**

ArchiveRoot: `Git history/by-date/2026-07-13/harness-report-root`。执行后 `reports/` 根直接文件目标为 12；三个归档组分别不超过 30；所有 manifest 源路径均不存在、目标均存在。
...
`HARNESS_DEBT.md` 将 DEBT-026、DEBT-027 标记为 fixed，并登记 commit/evidence；`docs/harness-maintenance/legacy-rules/changelog.md` 记录实现。压缩接近 200 行的 active 文档，保证 Harness 非脚本文本不超过 200 行。
