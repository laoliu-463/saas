...
> 关联文档：[03-Test与Real网关契约](../../03-Test与Real网关契约.md)、[05-接口与数据模型](../../05-接口与数据模型.md)、[09-真实SDK联调准备清单](../../09-真实SDK联调准备清单.md)、[16-数据口径 Open Questions 审计](../audits/16-数据口径OpenQuestions审计.md)、[15-real-pre后续联调执行清单](./15-real-pre后续联调执行清单.md)
...
**GMV 口径说明（排查必看）**：当前库内 GMV 来自 `colonelsettlement_order`（主同步 `buyin.instituteOrderColonel` 落库），不是仅依赖 `buyin.colonelMultiSettlementOrders`。多结算接口用于结算样本与 Webhook 定向同步补充，见 [04-开发进度](../../04-开发进度.md) 与 [16-数据口径审计](../audits/16-数据口径OpenQuestions审计.md)。
