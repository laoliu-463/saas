...
更新时间：2026-05-25（与 `docs/README.md`、`docs/04-开发进度.md` 事实口径对齐）
...
| `LogisticsGateway` | 物流轨迹查询、快递100订阅与发货履约辅助 | [x] 已完成，已补无轨迹 / 在途 / 签收 / 问题件 / 失败模拟底座 | [~] 快递鸟即时查询、快递100实时轨迹查询与快递100企业版订阅推送均已接入本地 Gateway；Sandbox / real-pre 尚未验证 |
...
- 物流能力仍挂在寄样 / 合作单履约下，前端和公开接口不新增独立物流业务域。
- 快递100作为寄样 / 合作单履约 provider，不新增独立物流业务域；优先使用企业版订阅推送，实时查询保留为手动刷新与兜底。
- 快递100配置统一走 `logistics.provider=kuaidi100` 与 `logistics.kd100.*`；旧 `kuaidi100.*` 不再作为读取口径。
- 快递100真实查询由 Gateway 生成 `customer + sign + param` 表单，`sign=MD5(param + key + customer)` 大写，并按 provider 专属映射转换快递公司编码。
- 快递100订阅由 Gateway 请求 `LOGISTICS_KD100_SUBSCRIBE_ENDPOINT`，回调入口为 `POST /api/public/logistics/kuaidi100/callback`，验签口径为 `MD5(param + callbackSalt).toUpperCase()`。
- 快递100回调以 `param.lastResult.state` 作为物流状态事实；`state=3` 只允许把仍处于待发货 / 快递中的寄样推进到待交作业，不触碰订单归因与业绩计算。
- test 环境诊断接口不得请求真实快递100 / 快递鸟上游；未配置凭证时返回 `NOT_CONFIGURED`。
