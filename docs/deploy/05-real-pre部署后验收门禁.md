...
仅当本轮验收目标包含商品库复制简介携带推广链接、真实转链或 `pick_source` 归因时执行本节。执行前必须确认 `APP_TEST_ENABLED=false`、`DOUYIN_TEST_ENABLED=false`、`DOUYIN_REAL_UPSTREAM_MODE=live`，并在人工批准写窗口内同时设置：
...
2. 后端日志出现 `promotion_convert_result=success`，并包含 `product_id`、`channel_id`、`pick_source`、`result=success`。
...
5. 前端复制文案包含 `【链接】` 推广链接；若未包含链接，只能记为转链失败兜底复制，不能记为真实转链 PASS。
