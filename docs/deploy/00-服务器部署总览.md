...
> **历史归档，禁止执行其中手工部署命令。** 当前唯一入口见 `docs/10-部署运行总览.md` 与 `harness/rules/cicd-real-pre-policy.md`。
...
| Jenkins | 唯一 real-pre 发布控制器：消费发布清单、拉取不可变镜像、锁定部署、迁移、验收、回滚和归档证据 | 不在服务器源码构建，不接受任务分支，不由 Agent 绕过队列 |
...
2. 仅在 Break-glass 获批时，按 [real-pre 单机受控部署手册](01-xshell-manual-deploy.md) 完成 Docker 端口部署。
...
7. 日常发布按 [07-Jenkins自动化部署规划.md](07-Jenkins自动化部署规划.md) 进入 GitHub 镜像产物和 Jenkins 发布队列；手工 SSH 只作为审批后的 Break-glass。
