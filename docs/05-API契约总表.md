...
- [V1 必做] `GET/PUT /api/products/{relationId}/sample-setting`：寄样设置以 `product_snapshot.id` 作为关系 ID，读写 `product_operation_state.audit_payload` 中的免费寄样开关、达人门槛、样品盒数和样品数量；PUT 必须保留同一商品已有审核补充字段，使用商品操作状态乐观锁并记录 `SAMPLE_SETTING` 操作日志，不改变商品审核或上架状态。
