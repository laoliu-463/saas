...
- [V1 必做] `GET/PUT /api/products/{relationId}/sample-setting`：寄样设置以 `product_snapshot.id` 作为关系 ID，读取和写入 `product_operation_state.audit_payload` 中的 `supportFreeSample`、`hasSampleThreshold`、`minWindowSales30d`、`minSales30d`、`minFans`、`minTalentLevel`（达人等级范围 `0-7`，前端展示为 `LV0-LV7`）；PUT 必须保留同一商品已有审核补充字段，使用商品操作状态乐观锁并记录 `SAMPLE_SETTING` 操作日志，不改变商品审核或上架状态。截图中的两个无字段名输入框可以编辑，但因当前没有字段契约，仅作为前端临时输入，不属于当前 API 字段，不提交到后端。
