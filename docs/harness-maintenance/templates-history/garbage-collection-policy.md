...
> `runtime/qa/out/*.md` 当前摘要和 `runtime/qa/out/` 唯一原始证据默认**受保护**；
> 任何归档 / 删除必须按 `docs/harness-maintenance/legacy-rules/runbooks/governance/debt-governance.md` 的"Report Rotation"流程走 manifest。
...
- 重复的环境说明，合并到 `docs/harness-maintenance/legacy-rules/environment/envs/*.md` 和 `docs/10-部署运行总览.md`。
