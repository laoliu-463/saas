...
| 优先级 | 路径 | 用途 |
| --- | --- | --- |
| 1 | `docs/harness-maintenance/engineering/issue-tracker.md` | Issue tracker 配置 |
| 2 | `docs/harness-maintenance/engineering/triage-labels.md` | 5 个 canonical 标签映射 |
| 3 | `docs/harness-maintenance/engineering/context.md` | Domain doc 消费规则 |
| 4 | `docs/harness-maintenance/engineering/issues-index.md` | GitHub Issues 镜像 |
| 5 | `AGENTS.md`（项目根）## 11. Agent skills section | skill 总览（指向 #1~#4） |
| 6 | `harness/README.md` | harness 总入口（包含 engineering/ 引用） |
...
```bash
# 1. 检查 4 个核心配置文件存在
ls -la docs/harness-maintenance/engineering/{issue-tracker,triage-labels,context,issues-index}.md

# 2. 检查 AGENTS.md 指向正确
grep -A 1 "## 11. Agent skills" AGENTS.md

# 3. 检查 harness/README.md 包含 engineering
grep "engineering/" harness/README.md
...
如果任何一步失败，按 `docs/harness-maintenance/engineering/` 下对应文件修复。
...
| 通过文件路径（如 `docs/harness-maintenance/engineering/issue-tracker.md`） | ✅ |
| 通过 AGENTS.md 中的 ## 11. Agent skills section | ✅ |
| 通过 harness/README.md | ✅ |
...
| 症状 | 排查路径 |
| --- | --- |
| "找不到 issue-tracker.md" | 1. 确认在正确仓库目录<br>2. 确认 `docs/harness-maintenance/engineering/` 目录存在<br>3. `find . -name "issue-tracker.md"` 全局搜索 |
...
| "docs/harness-maintenance/engineering/ 目录不存在" | 重新跑 setup-matt-pocock-skills skill |
