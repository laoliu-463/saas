...
| --- | --- | --- | --- | --- | --- |
| 后端功能修改 / 修复 | `CLAUDE.md`、`docs/README.md`、对应领域合同、`docs/05-API契约总表.md`、`docs/06-数据模型总表.md`、`docs/harness-maintenance/legacy-rules/runbooks/backend-change.md` | 对应流程、`docs/harness-maintenance/legacy-rules/skills/ddd/domain-alignment.skill.md` | `backend` | `agent-do.ps1 -Env real-pre -Scope backend` | Maven 构建、后端健康、相关 API/SQL、稳定 evidence |
...
| 远端发布 | `docs/harness-maintenance/legacy-rules/environment/envs/remote-real-pre-env.md`、`docs/harness-maintenance/legacy-rules/runbooks/remote-deploy.md`、`docs/harness-maintenance/legacy-rules/cicd-real-pre-policy.md` | `docs/10-部署运行总览.md` | Jenkins | 提升 PR 到 `release/real-pre`，由 `saas-real-pre-cd` 排队 | 完整 SHA、镜像摘要、运行版本、Flyway、发布清单与 Jenkins 报告 |
| 测试与验收 | `docs/09-测试验收总览.md`、`docs/验收/*.md`、`docs/harness-maintenance/legacy-rules/runbooks/governance/test-validation.md` | 对应 eval | 按任务 | `npm run e2e:v1-p0`、`npm run e2e:real-pre:p0:preflight`、`mvn -f backend/pom.xml test` | 测试报告、失败截图/日志、结论不能夸大 |
...
| 任务收尾与复盘 | `docs/harness-maintenance/legacy-rules/governance/session-exit-gate.md`、`docs/harness-maintenance/legacy-rules/feedback/iteration.md`、`docs/harness-maintenance/legacy-rules/runbooks/governance/closeout-and-gc.md` | `docs/harness-maintenance/legacy-rules/feedback/retire.md` | 当前 Scope | `agent-do.ps1`、`retire-content.ps1` | 稳定 evidence、内联 retro、旧内容候选、状态更新、剩余风险 |
| Git 工作区治理 / 批次提交 | `docs/harness-maintenance/legacy-rules/skills/git/git-change-control.md`、`docs/harness-maintenance/legacy-rules/skills/git/git-batch-submit.md`、`docs/harness-maintenance/legacy-rules/skills/workflow/post-task-gc.md` | `docs/harness-maintenance/legacy-rules/governance/COMPLETION_GATES.md`、`docs/harness-maintenance/legacy-rules/governance/session-exit-gate.md` | `docs` 或 `git-batch` | `git status`、`git diff --cached --check` | 详细见下表 |
...
1. `docs/harness-maintenance/legacy-rules/policies/agent-contract.md`
2. `docs/harness-maintenance/legacy-rules/governance/forbidden-scope.md`
3. `docs/harness-maintenance/legacy-rules/runbooks/ddd/DDD_OPTIMIZATION_ROADMAP.md`
4. `docs/harness-maintenance/legacy-rules/runbooks/ddd/DDD_DOMAIN_TASK_MATRIX.md`
5. 当前领域对应的 `docs/harness-maintenance/legacy-rules/instructions/domain/*.md`
...
| --- | --- | --- |
| 登录、权限、菜单、角色、组织、数据范围 | 用户域 | `docs/harness-maintenance/legacy-rules/instructions/domain/user-domain.md` |
| 系统配置、规则参数、提成比例、复制模板、`pick_extra` | 配置域 | `docs/harness-maintenance/legacy-rules/instructions/domain/config-domain.md` |
| 订单同步、`pick_source`、`colonel_buyin_id`、默认归因、`raw_payload`、双轨金额输入 | 订单域 | `docs/harness-maintenance/legacy-rules/instructions/domain/order-domain.md` |
| `performance_records`、提成、冲正、最终归属 | 业绩域 | `docs/harness-maintenance/legacy-rules/instructions/domain/performance-domain.md` |
| 看板、汇总表、趋势、排行、对账 | 分析模块 | `docs/harness-maintenance/legacy-rules/instructions/domain/analytics-module.md` |
| 商品库、活动商品、展示规则、转链、`pick_source_mapping` | 商品域 | `docs/harness-maintenance/legacy-rules/instructions/domain/product-domain.md` |
| 达人、认领、保护期、地址、标签 | 达人域 | `docs/harness-maintenance/legacy-rules/instructions/domain/talent-domain.md` |
| 寄样申请、审核、发货、签收、待交作业、交作业完成 | 寄样域 | `docs/harness-maintenance/legacy-rules/instructions/domain/sample-domain.md` |

### DDD 执行口径

- 单个任务只允许推进一个主责领域的一个任务卡；跨域影响必须记录为依赖，不得顺手重构。
- 修改前必须执行 `docs/harness-maintenance/legacy-rules/skills/ddd/ddd-boundary-check.skill.md` 的边界检查清单。
- 修改后必须按当前 Scope 执行固定入口；文档任务使用 `Scope=docs`。
- 任务完成后必须更新 `docs/harness-maintenance/legacy-rules/state/snapshots/DOMAIN_STATUS.md`、相关 state 和 feedback；没有证据不得把任务状态写成完成。
...
Gate 定义和验证要求见 `docs/harness-maintenance/legacy-rules/governance/COMPLETION_GATES.md`。
...
Session Exit Gate 定义和检查模板见 `docs/harness-maintenance/legacy-rules/governance/session-exit-gate.md`。

## Git 任务路由

Git 类任务必须按以下子任务路由执行，先读 `docs/harness-maintenance/legacy-rules/skills/git/git-change-control.md` 和 `docs/harness-maintenance/legacy-rules/skills/git/git-batch-submit.md`。
