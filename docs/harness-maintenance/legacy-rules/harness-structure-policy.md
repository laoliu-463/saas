...
- 非脚本文本 160 行预警、200 行硬上限；仅 `harness/package-lock.json` 精确豁免行数预算，其他 JSON/lockfile 不豁免；脚本改由测试、语法检查和职责边界约束。
...
- Git 忽略的 `harness/node_modules/` 不参与目录或仓库健康统计且禁止提交；其他未知一级目录仍立即失败。
