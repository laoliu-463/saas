...
| 验收证据 | keep / retention 后 archive | `runtime/qa/out/`、`runtime/qa/out/` |
| 源码旧实现 | delete only after full validation | 已被新实现替代且测试证明无引用 |

## 归档规则

- 归档必须使用 manifest。
- 默认归档到按日期/主题分桶的 `Git history/` 子目录；同一分桶直接文件和子目录均不得超过 50。
- 归档前必须确认新主源已存在，并且引用入口已更新。
- 归档报告覆盖 `runtime/qa/out/latest-content-retire.md`。
