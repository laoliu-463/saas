...
raw payload 或商品快照中的费率提示字段（小数，如 0.05 = 5%）。当前 `ColonelsettlementOrder` 中这两个字段是非落库展示字段；订单金额事实落库不得把缺失 rate 静默写成 0，也不得把 rate 当成结算事实。
...
**estimateServiceFeeExpense（预估服务费支出）** / **effectiveServiceFeeExpense（已结服务费支出）**:
