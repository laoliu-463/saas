...
-- =============================================
ALTER TABLE colonelsettlement_order
    ADD CONSTRAINT chk_cso_status
        CHECK (status IN ('PAID', 'SETTLED', 'REFUNDED', 'CANCELLED'))
        NOT VALID;

ALTER TABLE colonelsettlement_order
    ADD CONSTRAINT chk_cso_attribution_status
        CHECK (attribution_status IN ('UNATTRIBUTED', 'ATTRIBUTED', 'EXCLUSIVE', 'AMBIGUOUS', 'FAILED'))
        NOT VALID;

ALTER TABLE colonelsettlement_order
    ADD CONSTRAINT chk_cso_order_type
        CHECK (order_type IN ('MAIN', 'SETTLEMENT'))
        NOT VALID;
