...
2. 联调接口（后端）
- `POST /api/douyin/token/exchange-code`
- `POST /api/douyin/token/refresh`
- `GET /api/douyin/token/status`
- `POST /api/douyin/token/bootstrap`
- `POST /api/douyin/activity/create-or-update`
...
1. `/token/create`（无需 access_token）
...
2. `/token/refresh`（无需 access_token）
...
执行策略：
- 仅在“无 access_token”或“token 即将过期”时调用 `/token/refresh`
...
- 终止合作接口：`alliance.colonelActivityProductCancel`
  - 新增联调入口：`POST /api/douyin/activity/product/cancel`
...
- 开放消息回调：`doudian_alliance_colonelOpenEvent`
  - 新增接收端点：`POST /api/douyin/webhook/colonel-open-event`
  - 默认不强制验签；可通过 `DOUYIN_WEBHOOK_VERIFY_SIGN=true` 开启验签

## 12. 订单同步接口迁移（2026-04-22）
