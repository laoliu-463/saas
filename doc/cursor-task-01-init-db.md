...
| 优先级 | P0 |
| 预计工时 | 2h |
| 前置依赖 | PostgreSQL 15+ 实例已就绪 |
| 输出产物 | `backend/src/main/resources/db/init-db.sql` |

---

## 1. 目标

在 Spring Boot 项目中创建完整的数据库初始化脚本，覆盖 V1.3 数据库设计的全部 20 张表（含分区表），并提供分区自动管理函数和种子数据。

> **V1.3 修订**：基于需求文档+API文档交叉比对，补充 6 项 P0/P1 修正和 5 项 P2 改进 + 7 项V1.3关键字段补充。

---

## 2. 文件输出规范

- 输出路径：`backend/src/main/resources/db/init-db.sql`
- 编码：UTF-8
- 每条 DDL 语句独立，用注释分隔模块
- 脚本必须 **幂等可重入**：使用 `CREATE TABLE IF NOT EXISTS` / `CREATE INDEX IF NOT EXISTS`
...
将以下内容写入 `backend/src/main/resources/db/init-db.sql`：
...
## 4. 与 V1.1 设计的差异说明（微调修正）

| 修正点 | 原设计 | 修正后 | 原因 |
|--------|--------|--------|------|
| order_detail.order_id 索引 | 普通 INDEX | UNIQUE INDEX | 一个订单只有一条解密信息 |
| operation_log 分区 | 仅到 2026-12 | 延伸到 2027-03 | 与 colonelsettlement_order 对齐 |
| DDL 幂等性 | 无 | 全部加 `IF NOT EXISTS` | 支持重复执行不报错 |
| 种子数据 | 仅 system_config | 增加 sys_role 默认5个角色 | 项目启动必须 |
| 分区自动管理 | 无 | 新增 `create_next_month_partitions()` 函数 | 自动运维，避免手动建分区 |
| **sys_user 缺 channel_code** | 无 | 新增 `channel_code VARCHAR(16) UNIQUE` | **P0** pick_extra 限20字符，UUID超限 |
| **sys_role 缺权限配置** | 仅有 data_scope | 新增 `permissions JSONB` + `menu_config JSONB` | **P1** 需求§2.3菜单/操作权限可配 |
| **talent 缺 crawl_status** | 无 | 新增 `crawl_status` + `crawl_message` | **P1** 追踪采集状态，支持失败兜底 |
| **sample_request 缺收件地址** | 无 | 新增 `recipient_name/phone/address` | **P1** 需求§3.5填写收货地址 |
| **pick_source_mapping 缺唯一约束** | 普通 INDEX | UNIQUE INDEX | **P1** 归因码唯一，防重复插入 |
| **pick_source_mapping.pick_source 过短** | VARCHAR(20) | VARCHAR(128) | **P2** API实际返回可能较长 |
| **colonel_activity 缺 colonel_buyin_id** | 无 | 新增 `colonel_buyin_id BIGINT` | **P1** 订单归因需匹配团长百应ID |
| **colonel_activity 缺费率字段** | 无 | 新增 `commission_rate` + `service_rate` + `last_sync_at` | **P2** 活动门槛值+同步追踪 |
| **douyin_token 缺 extra_data** | 无 | 新增 `extra_data JSONB` | **P2** 存储scope/shop_biz_type等扩展字段 |
| **pick_source_mapping 缺方案B字段** | 无 | 新增 `short_id VARCHAR(10) UNIQUE` + `uuid_seed UUID` + 索引 | **P0** 方案B ShortID透传机制，转链接口依赖 |
| **pick_source_mapping.pick_extra 过大** | VARCHAR(50) | VARCHAR(10) | **P0** 实际透传值=short_id≤10字符 |
| **colonelsettlement_order 缺金额字段** | 无 | 新增 `order_amount BIGINT` + `actual_amount BIGINT` | **P0** 订单金额/实际结算金额（分） |
| **product 缺分佣字段** | 无 | 新增 `cos_ratio/cos_fee/service_ratio` | **P0** 商品级分佣比例/金额 |
| **merchant 缺状态字段** | 无 | 新增 `status SMALLINT DEFAULT 1` | **P0** 商家启用/禁用控制 |
| **douyin_token 缺 app_id** | 无 | 新增 `app_id VARCHAR(100)` | **P1** 抖店应用ID标识 |
| **colonel_activity_product 缺 min_refer_amount** | 无 | 新增 `min_refer_amount BIGINT` | **P1** 最低推广金额门槛（分） |
...
```bash
# 1. 表数量（预期 20 张：18 普通表 + 2 分区主表）
SELECT count(*) FROM pg_tables WHERE schemaname = 'public';

# 2. 分区子表数量（预期 24：订单12 + 日志12）
SELECT count(*) FROM pg_inherits;

# 3. 种子数据
SELECT count(*) FROM system_config;  -- 预期 13
SELECT count(*) FROM sys_role;        -- 预期 5

# 4. 唯一约束检查
SELECT indexname, tablename FROM pg_indexes
 WHERE indexname LIKE 'uk_%' ORDER BY tablename;
```
...
| 序号 | 任务 | 类型 | 依赖 |
|------|------|------|------|
| TASK-02 | Spring Boot 项目骨架 + MyBatis-Plus 配置 | B | TASK-01 |
| TASK-03 | Entity 实体类生成（20张表） | B | TASK-01 |
| TASK-04 | 基础 CRUD Service/Controller | B | TASK-02, TASK-03 |

---

*流程指挥官生成 | 2026-04-20 | V1.3修订（V1.2+关键字段补充）| 状态：待执行*
