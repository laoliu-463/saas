...
function Get-HarnessAutoScope {
    ...
    foreach ($file in $changed) {
        ...
        $isSimpleDocs = $file -match '^(README[^/]*|CONTRIBUTING\.md|docs/|docs\\|harness/reports/current/)' -or
            $file -match '\.(md|txt)$'
...
function Invoke-HarnessNodeVerify {
    ...
    $snapshot = Get-HarnessNodeGitSnapshotJson -RepoRoot $repoRoot
    ...
    try {
        ...
        $env:HARNESS_VERIFY_GIT_SNAPSHOT_JSON = $snapshot
