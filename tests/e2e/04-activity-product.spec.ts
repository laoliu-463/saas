...
test('活动列表和活动商品页可展示', async ({ page }, testInfo) => {
  ...
  await expect(page.getByTestId('product-action-assign-audit-owner').first()).toBeVisible();
  await expect(page.getByTestId('product-action-detail').first()).toBeVisible();
  await expect(page.getByTestId('product-action-logs').first()).toBeVisible();
