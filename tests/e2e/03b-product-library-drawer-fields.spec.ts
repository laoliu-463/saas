 ...
 * {@code shopScore}，导致商品库卡片 hover 抽屉"活动"字段一直为 {@code -}，且没有"店铺评分"字段。
 * 本测试覆盖：</p>
 * <ol>
 *   <li>抽屉中"店铺评分"字段存在并显示真实分值（来自后端 rawPayload.shopScore）</li>
...
test('商品库抽屉显示店铺评分 + 活动字段', async ({ page }, testInfo) => {
  ...
  // 抽屉里"店铺评分"和"活动"两个字段都应该渲染
  await expect(drawer).toContainText('店铺评分');
